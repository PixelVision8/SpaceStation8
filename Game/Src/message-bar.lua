...
function MessageBar:Init(y, height, textYOffset)
    ...
    local messageBar = {
        ...
        font = "medium",
        pos = NewPoint(0, y),
        textPos = NewPoint(0,y + textYOffset),
...
function MessageBar:DisplayMessage(value, time, centered, onClearCallback)
    ...
    -- Since we have changed the text and updated the time, we need to redraw the menu so it will render on correctly. We start by drawing a rectangle behind the message text to clear the menu area.
    DrawRect( 
        self.pos.x,
        self.pos.y,
        self.maxChars * 4,
